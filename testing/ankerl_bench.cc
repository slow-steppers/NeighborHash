...
using neighbor_hash_map = neighbor::NeighborHashMap<
  uint64_t, uint64_t, neighbor::policy::DefaultPolicy<uint64_t, uint64_t>>;
TEST_CASE_MAP("bench_find_random_nb" * doctest::test_suite("bench") * doctest::skip(), size_t, size_t) {
    bench<neighbor_hash_map>();
}

TEST_CASE_MAP("bench_find_random_flat" * doctest::test_suite("bench") * doctest::skip(), size_t, size_t) {
