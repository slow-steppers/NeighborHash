...
class Slot {
 public:
  ...
  void set_payload(mapped_type payload) {
    data_.second = (payload & MapType::kPayloadMask) | (data_.second & MapType::kOffsetMask);
  }

  void set_offset(int offset) {
    data_.second = static_cast<mapped_type>(
  ...
  int offset() const {
    return static_cast<typename std::make_signed<mapped_type>::type>(data_.second)
        >> MapType::kPayloadBitCount;
  }

  mapped_type payload() const {
    return data_.second & MapType::kPayloadMask;
  }
